...
import { loadFonts } from "@/shared/utils/fonts";
...
export const FabricCanvas: FC<Props> = ({
    ...
    const loadObjects = async (canvas: Canvas) => {
        ...
        const fontsData = objectsFonts.map((objectFont) => {
            const obj = initialObjects.find(
                (obj) => obj.fontFamily === objectFont,
            );
            if (obj) {
            ...
            const defaultFont = FONTS.find(
                (font) =>
                    font.family === objectFont ||
                    font.postScriptName === objectFont,
            );
            return {
